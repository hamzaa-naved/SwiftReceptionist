    ...
    <>
      <Section className="pt-32 md:pt-40">
        ...
        {caseStudies.length > 0 ? (
            ...
            {caseStudies.map((cs, i) => (
              <Reveal key={cs.business} delay={i * 0.06} className="h-full">
                <TiltCard className="h-full rounded-3xl">
                  <article className="h-full rounded-3xl border border-line bg-white p-8 shadow-card md:p-10">
                  <p className="eyebrow text-azure-600">
                    {cs.niche} · {cs.location}
                  </p>
                  <h2 className="mt-4 text-2xl font-semibold tracking-[-0.01em] text-carbon-950">{cs.business}</h2>
                  <blockquote className="mt-4 border-l-2 border-azure-600 pl-4 text-lg leading-snug text-carbon-600">
                    “{cs.quote}”
                  </blockquote>
                  <dl className="mt-6 grid grid-cols-2 gap-4">
                    {cs.metrics.map((m) => (
                      <div key={m.label}>
                        <dt className="text-xs uppercase tracking-wider text-carbon-400">
                          {m.label}
                        </dt>
                        <dd className="font-display text-3xl text-carbon-950">
                          {m.value}
                        </dd>
                      </div>
                    ))}
          ...
          <div className="relative mx-auto max-w-2xl rounded-3xl border border-line bg-white p-10 text-center shadow-card">
            ...
            <p className="mx-auto mt-4 max-w-md text-sm font-medium text-carbon-950">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {tracked.map((t, i) => (
            <Reveal key={t.label} delay={i * 0.05} className="h-full">
              <TiltCard className="h-full rounded-3xl">
                <div className="h-full rounded-3xl border border-line bg-white p-7 shadow-card">
                  <span className="mb-5 inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-cloud">
                    <t.icon className="h-5 w-5 text-azure-600" strokeWidth={2} aria-hidden />
                  </span>
                  <h3 className="mb-2 text-lg font-semibold tracking-[-0.01em] text-carbon-950">{t.label}</h3>
                  <p className="text-sm leading-relaxed text-carbon-600">{t.detail}</p>
                </div>
              </TiltCard>
            </Reveal>
          ))}
