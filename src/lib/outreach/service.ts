...
export async function getOrCreateDemo(lead: Lead): Promise<Demo> {
  ...
  if (row) return { id: row.id, leadId: lead.id, token: row.token, url: `${BASE_URL}/demo/p/${row.token}`, expiresAt: row.expires_at };
  ...
  return { id, leadId: lead.id, token, url: `${BASE_URL}/demo/p/${token}`, expiresAt };
}
...
export async function previewBatch(batchId: string) {
  ...
  const previews = rows.map((row) => {
    ...
    const demo: Demo = { id: String(typed.demo_id), leadId: lead.id, token: String(typed.token), url: `${BASE_URL}/demo/p/${typed.token}`, expiresAt: String(typed.expires_at) };
